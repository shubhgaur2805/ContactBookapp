...
def up_image():
	...
	#show_img=PhotoImage(file=Entry_image)
	#lb_img.config(image=show_img)
	#on=PhotoImage(file=Entry_image)
	#lb_img=Label(root,image=on)
	#lb_img.pack()#grid(row=7,column=2,padx=10,pady=5)
...
def import_data():
	pass
	"""
	open_file=filedialog.askopenfilename(initialdir="/tkinter/img",title="Import Data",filetypes=(("Excel files","*.xlsx"),("CSV files","*.csv"),("All Files","*.*")))
...
save_to_excel=Button(frame_1,text="Save_To_Excel",command=excel_data,bg="peach puff",relief=GROOVE,bd=10,width=12).grid(row=5,column=7,padx=10,pady=5)
import_data_button=Button(frame_1,text="Import Data",command=import_data,bg="peach puff",relief=GROOVE,bd=10,width=15).grid(row=6,column=6,columnspan=2,pady=10,padx=5)
add_data_tree()
